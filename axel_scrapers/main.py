...
# imports for individual scrapers
from scrapers.apple import AppleScraper
from scrapers.lenovo import LenovoScraper
from scrapers.dell import DellScraper
from scrapers.hp import HPScraper

# load individual URL's
def load_start_urls(path: str) -> list[str]:
    with open(path, "r", encoding="utf-8") as f:
        return [line.strip() for line in f if line.strip()]

# ensure metadata directories for csv's
...
def run_scraper(
    # inputs for other classes/functions
    name: str,
    scraper_cls,
    config_file: str,
    meta_filename: str,
